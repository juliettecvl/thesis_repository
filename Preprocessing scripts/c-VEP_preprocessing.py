...
# Define directories
data_dir = '/Users/juliette/Desktop/Radboud/work/BCI/cVEP'
thesis_dir = '/Users/juliette/Desktop/thesis/preprocessing'
save_dir = '/Users/juliette/Desktop/thesis/preprocessing/c-VEP_preprocessing'
...
l_freq = 6  # highpass cutoff in Hz
h_freq = 30  # lowpass cutoff in Hz
...
for i_subject, subject in enumerate(subjects):
    ...
    for i_task, task in enumerate(tasks):
        ...
        for i_run, run in enumerate(runs[task]):
            ...
            # Run XDF file name
            fn = os.path.join(data_dir, f"sub-{subject}", "ses-S001", "eeg",
        ...
        # Save data
        save_dir = os.path.join(data_dir, "derivatives", f"sub-{subject}")
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        np.savez(os.path.join(save_dir, f"sub-{subject}_task-{task}.npz"), X=X, y=y, z=z, V=V, fs=fs)
